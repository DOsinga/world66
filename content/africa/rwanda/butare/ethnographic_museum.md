---
title: Ethnographic Museum
type: poi
tags:
  - things_to_do
  - museum
latitude: -2.5887
longitude: 29.7451
story: >
  The museum was built as Rwanda's national museum before the country's museum network
  expanded. Its collections still make it one of the best places to understand
  pre-colonial Rwandan social life, craft, architecture, music, and ritual objects in
  one coherent visit.
sources:
  - https://en.wikipedia.org/wiki/Ethnographic_Museum_(Rwanda)
  - https://www.petitfute.com/v45359-butare-huye/c1173-visites-points-d-interet/c958-musee/155000-musee-ethnographique.html
---

The Ethnographic Museum is Butare's essential stop and one of the strongest museums in East Africa for anyone trying to understand Rwanda beyond headlines. The galleries cover domestic life, agriculture, music, ritual, kingship, crafts, and architecture, with enough depth to reward more than a quick circuit.

Give it two hours if you can. A guide adds useful context, especially for objects whose meaning is not obvious from the label. The grounds are calm too, so this is a good place to let the visit breathe rather than ticking it off between bus rides.
