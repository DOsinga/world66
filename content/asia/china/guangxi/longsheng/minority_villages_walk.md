---
title: Minority Village Walk Between Ping'An and Dazhai
type: poi
tags:
  - things_to_do
  - sight
latitude: 25.7900
longitude: 110.0100
score: 8.2
---

The full walk between the Ping'An (Zhuang) and Dazhai (Yao) terrace areas, crossing the ridge between the two valleys, takes about four to five hours at a comfortable pace. The path runs through tea plantations, bamboo groves, and several small villages that see a fraction of the visitors that pass through the two main terrace zones. The views from the ridge connecting the two areas — looking east over the Ping'An terraces and west over the Jinkeng formation simultaneously — are the best available in the whole scenic area.

This walk requires some planning: you start in one car park and end in the other, so transport from town and back needs to be arranged in advance, or you walk back to where you started (add another two to three hours). Most guesthouses in the village areas can help arrange minivan transport between car parks for the end of the day. The path is well-trodden but steep in sections; poles are useful for the descents.
