---
title: "The Five Linked Lakes"
type: poi
tags:
  - things_to_do
  - sight
  - nature
latitude: 48.6700
longitude: 126.1300
score: 8.1
---

The five interconnected lakes that give Wudalianchi its name — wudalianchi means literally "five large linked pools" — were created when the 1719–1720 lava flows dammed the Bai River. The resulting chain of lakes stretches across the volcanic landscape, their blue water in striking contrast to the black basalt that surrounds them. Boat trips on the lakes operate in summer, offering views of the volcanic cones from the water and a perspective on the scale of the 300-year-old geological event.

The forest that has re-established itself around the lake shores since the eruption is now mature enough to provide pleasant walking conditions, and the network of trails between the lakes and the volcanic cones can fill a full day comfortably. In winter the frozen lakes and snow-covered lava fields create a landscape of austere monochrome beauty — less visited than in summer but striking in its own way.
