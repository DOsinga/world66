---
title: "Shankaracharya Hill Walk"
type: poi
tags:
  - things_to_do
  - sight
latitude: 34.0902
longitude: 74.8391
score: 8.4
---

The path up Shankaracharya Hill, the 243 steps from the vehicle checkpoint to the temple at the summit, is one of the better short climbs available from the city. The steps are well-maintained stone, shaded for much of the way by trees, and the ascent takes 15 to 25 minutes depending on pace. Along the way, the city progressively reveals itself — the lake appears first at a gap in the trees, then the whole of central Srinagar comes into view, and finally from the summit the panorama extends to the Pir Panjal range to the south and the high peaks around Gulmarg visible on clear days.

The hill is called Takht-e-Suleiman — Throne of Solomon — in local usage, and the path used by pilgrims and visitors follows what is probably the oldest religious route in Kashmir. The earliest written references to worship on this hill predate the Common Era. Security personnel at the checkpoint inspect bags and the approach road is monitored; photography near the military installations on the hillside is restricted, but the view from the temple itself and the approach path are freely photographable. Evening is a popular time for the climb with the sunset view over the lake.
