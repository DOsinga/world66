---
title: "Victoria Bridge"
type: poi
latitude: 32.9192
longitude: 73.7364
tags:
  - things_to_do
  - sight
score: 6.4
---

The Victoria Bridge carries the railway across the Jhelum River on a series of latticed iron spans that have been doing this job since the late 19th century, when it was built as part of the North Western Railway's main line between Lahore and Peshawar. It is still in active use — trains pass over it several times a day — and the ironwork, though well over a century old, retains the confident over-engineering of the Victorian railway builders. From the river's edge below the bridge you can see the full length of the spans and appreciate the scale of the crossing: the Jhelum runs wide here after descending from the mountains of Kashmir.

The best views are from the riverbank on either side of the bridge, where local fishermen keep their boats. In the late afternoon, when the light comes from the west and the river catches it, the scene is worth photographing. The bridge itself is not publicly walkable — it is a working railway bridge — but the approach roads on both banks give you close access. The river here is slow-moving and wide, which makes the bridge look more substantial than a narrow mountain gorge crossing would; you are seeing a lowland river that was once broad enough to have determined the course of an empire.
