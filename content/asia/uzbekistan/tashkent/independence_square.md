---
title: "Independence Square"
type: poi
snippet: "Vast central square — formerly Lenin Square — with fountains and a globe"
tags:
  - things_to_do
  - sight
  - new_town
neighbourhood: New Town
latitude: 41.3157
longitude: 69.2693
score: 6.2
---

Tashkent's biggest public space, formerly Lenin Square. The colossal Lenin statue that once dominated it was pulled down in 1992 and replaced with a globe topped by an outline of Uzbekistan. The square is fringed by government buildings, fountains, and a triumphal arch with metal storks.

A monument to the Mourning Mother on the eastern side, with an eternal flame, commemorates Uzbek soldiers killed in the Second World War. The whole space is most pleasant at night, when the fountains run and locals come out to walk.
