---
blogs:
- author: Alice Elizabeth Taylor
  name: The Balkanista
  note: Written from Tirana by a journalist who lives there. As much about Albanian
    politics, culture and daily life as about travel — which is exactly why it's worth
    reading first.
  url: https://thebalkanista.com/
- blog: Anita Hendrieka
  name: 19 Best Things to Do in Tirana, Albania I'd Actually Recommend
  note: A New Zealander who has lived in Albania full-time since 2017, based in Saranda,
    updating her guides monthly from inside the country.
  url: https://www.anitahendrieka.com/things-to-do-in-tirana-albania/
- name: Into Albania
  note: A whole-country guide to museums, hikes, festivals and castles, with the useful
    habit of covering places that have no tourism infrastructure yet.
  url: https://intoalbania.com/
- name: The Travelling Light
  note: A small personal blog whose Albanian road trip, Tirana included, reads like
    an actual trip rather than a itinerary template.
  url: https://thetravellinglight.com/
score: 8.0
snippet: A journalist in Tirana, a New Zealander who moved to Saranda and stayed,
  and three others who treat the capital as more than the airport you leave
title: 4 Blogs That Get Albania Right
type: bloglist
---

Albania went from "the poorest country in Europe and one of the least visited" to a beach-listicle staple in about five years, and most of what's written about it now is about the coast. Tirana gets three lines and a photo of a painted apartment block. These five do better: the communist-era bunkers and the House of Leaves, the Blloku district that ordinary Albanians weren't allowed to enter until 1991, the mountain roads north to Theth, and the politics that shape all of it.

Two of these writers live in Albania full-time and update from the ground, which matters in a country where the bus timetable, the road surface and the entry rules all change faster than the guidebooks.
