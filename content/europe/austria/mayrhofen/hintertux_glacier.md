---
title: Hintertux Glacier
type: poi
score: 8.0
snippet: High glacier ski area reachable from Mayrhofen through the Tux valley.
tags:
  - things_to_do
  - sight
  - skiing
  - winter_sports
sources:
  - https://en.wikivoyage.org/wiki/Mayrhofen
  - https://en.wikipedia.org/wiki/Mayrhofen
latitude: 47.055
longitude: 11.68
---

Hintertux Glacier is not in Mayrhofen, but it is part of why Mayrhofen works as a serious mountain base. The glacier sits high above the Tux valley and extends the area's ski logic beyond the normal winter season.

Treat it as a day trip from town rather than a casual stroll from the lift base. It is especially useful when lower slopes are thin or when you want the drama of glacier terrain.
