---
title: Cipières
type: poi
tags:
  - things_to_do
  - sight
latitude: 43.7842
longitude: 6.9558
sources:
  - https://en.wikipedia.org/wiki/Cipi%C3%A8res
score: 6.4
---

Cipières is a small hilltop village about two kilometres east of Gréolières, sitting on its own rocky spur at around 700 metres. The two villages make a natural pair: both are perched, both are medieval in origin, and both are quiet enough that you can walk the entire length of the main street without meeting another tourist. The views from Cipières look south across the plateau toward Grasse and the coast, and east along the valley toward Vence.

The village has a Romanesque church and the usual complement of stone houses, some inhabited, some half-ruined. It is smaller than Gréolières and receives fewer visitors, which makes it appealing if you want to see what these pre-Alps villages looked like before the renovation wave that tidied up the more accessible ones. The road between the two villages is narrow and occasionally single-track but navigable by car.
