---
title: Enguri Gorge
type: poi
tags:
  - things_to_do
  - sight
latitude: 42.84
longitude: 42.60
story: >
  The Enguri Dam, completed in 1987 below the gorge, is one of the tallest
  arch dams in the world — 271 metres high, built across a gorge where
  the reservoir now backs up into the mountains. The dam's construction
  required relocating several villages and flooding parts of the lower
  gorge. The reservoir is visible from the road into Svaneti; the dam wall
  itself can be viewed from a bridge just above it.
---

The Enguri Gorge is the route into Upper Svaneti from the lowlands — a deeply cut river canyon running north from the Enguri Dam toward Mestia. The road from [Zugdidi](/europe/georgia/zugdidi) follows the river, climbing steeply through increasingly dramatic scenery: the gorge walls close in, waterfalls appear on the cliffs above, and the vegetation changes from oak forest to alpine meadows. The drive from Zugdidi to Mestia takes approximately 2.5-3 hours on paved road (mostly, with some rough sections).

The gorge is the main access route to Svaneti and is driven by the marshrutka (shared minibus) from Zugdidi that serves as the main public transport connection. The drive is worth doing in daylight: the landscape changes from the subtropical lowlands around Zugdidi to mountain terrain in the space of an hour. The Enguri Dam viewpoint is accessible from a side road about 40 kilometres north of Zugdidi.
