---
latitude: 41.6920
longitude: 44.8060
snippet: Popular wine shop and bar in the old town stocking Georgian natural wines
tags:
- bars_and_cafes
- bar
title: Vino Underground
type: poi
score: 7.6
---

Vino Underground on Galaktion Tabidze Street is one of the best wine bars in Tbilisi for natural and traditional-method Georgian wines, with a well-curated cellar stocking small producers from Kakheti, Kartli, and Imeretia alongside imports. The space is small and underground (as the name suggests), with a bar and a few tables; the staff know the wines well and are used to guiding non-specialist visitors.

This is the place to try the full range of Georgian wine styles in one sitting: light, European-style whites alongside skin-contact amber wines, earthy reds from Saperavi grapes, and the occasional experimental batch from a small natural producer. The food menu is minimal — mostly charcuterie and cheese — which keeps the focus where it should be.
