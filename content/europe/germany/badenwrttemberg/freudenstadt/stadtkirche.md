---
done:
  poi_cleanup: '2026-04-19'
latitude: 48.4631
longitude: 8.4094
snippet: Unique L-shaped Protestant church with twin naves
story: 'The church''s L-shaped plan was a deliberate solution to an early Protestant
  theological problem: how do you seat a congregation in a church without privileging
  either the pulpit or the altar? Heinrich Schickhardt''s answer was to build two
  identical wings meeting at a right angle, with both pulpit and altar placed at the
  inner corner so they are visible from both naves simultaneously.

  '
tags:
- things_to_do
- church
title: Stadtkirche
type: poi
score: 8.4
---

The parish church was designed by Heinrich Schickhardt and built between 1601 and 1608 as part of the original town plan. What makes it remarkable is its form: two identical naves meeting at a right angle, a layout unique in Protestant church architecture. The church was gutted in the 1945 bombardment and rebuilt, and while the original stone sculptures were lost, the spatial logic of the building survived intact. It sits at the southwest corner of the Marktplatz and is easy to combine with a walk around the square.
