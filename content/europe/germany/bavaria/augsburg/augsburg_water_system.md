---
latitude: 48.3655
longitude: 10.902
snippet: UNESCO-listed Renaissance water management system still supplying the city
story: Augsburg's water management system has been in continuous operation since the
  14th century and is listed as a UNESCO World Heritage Site — one of the few pieces
  of urban infrastructure to receive that designation. The system supplied clean water
  to the city's fountains (including the three famous Renaissance fountains on Maximilianstrasse)
  through a network of canals, water towers, and pumping houses. The three historic
  water towers near the Rotes Tor still stand and can be visited.
tags:
- things_to_do
- sight
title: Augsburg Water Management System
type: poi
score: 9.0
---

Augsburg's historic water supply system, a UNESCO World Heritage Site since 2019, covers the network of canals, water towers, and pumping stations that have supplied the city since the medieval period. The system was one of the most sophisticated in Europe when it was built, and several of its original structures survive: the three water towers near the Rotes Tor (Red Gate) can be explored, and the canal system still flows through the old town. A free trail map from the tourist office connects the main surviving elements on a 90-minute walking route.
