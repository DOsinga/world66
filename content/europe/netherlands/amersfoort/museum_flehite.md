---
address: Westsingel 50
latitude: 52.1568
longitude: 5.3856
phone: 033-4619987
snippet: Regional history museum in three medieval canal houses
story: >
  The museum occupies three adjoining medieval canal houses that once belonged to wealthy
  Amersfoort merchants. The name comes from "Flehite", the Latin name for the Eemland
  region, used by medieval scholars. Inside, a section is dedicated to the Amersfoort
  Stone — a large glacial boulder that in 1661 was bet by hundreds of citizens dragged
  into the city as a prank by a neighbour, and which became a symbol of civic
  gullibility so enduring that Amersfoort people are still called "Keienaars" (stone folk) today.
tags:
- things_to_do
- museum
title: Museum Flehite
type: poi
sources:
  - https://en.wikipedia.org/wiki/Museum_Flehite
score: 8.1
---

The regional history museum of Amersfoort and the Eemland region, housed in three beautifully preserved medieval canal houses on the Westsingel. The collection spans Roman times through the Middle Ages and up to the twentieth century — well-organised and accessible even without Dutch. The star exhibit is a section on the Amersfoort Kei (Stone), a legendary glacial boulder that became a symbol of the city. Don't miss the medieval town scale model in the basement that shows just how little the street plan has changed in 600 years.
