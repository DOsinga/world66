---
title: Museum of Contemporary Art
type: poi
latitude: 44.8194
longitude: 20.4422
tags:
  - things_to_do
  - museum
  - art_deco
story: >
  The building itself is a landmark of Yugoslav modernism — a white interlocking-cube
  structure designed by Ivan Antić and Ivanka Raspopović and opened in 1965.
  After two decades of closure for restoration, it reopened in 2017 with a
  new permanent display of Yugoslav and Serbian art from 1900 to the present,
  covering both the socialist realism period and the experimental work that pushed
  against its constraints.
score: 8.3
---

The Museum of Contemporary Art in New Belgrade — on the Sava riverbank, near the spot where the river meets the Danube — holds the most important collection of modern and contemporary Serbian art. The collection includes over 35,000 works, from early 20th-century painting through the Yugoslav-era socialist art to the experimental and conceptual work of the 1960s and 70s. The EXAT 51 group's geometric abstraction and the OHO movement's conceptual work are particularly well-represented.

The museum's location in New Belgrade, across the Sava from the old city, means it is often skipped by short-stay visitors. This is a mistake: the building, the collection, and the riverfront setting combine to make it one of the more memorable museum experiences in the region. Reachable by bus from Republic Square or on foot across Branko's Bridge in about twenty minutes.
