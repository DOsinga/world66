---
title: Viaduct of Pontevedra
type: poi
tags:
  - things_to_do
  - sight
  - architecture
latitude: 42.2845
longitude: -8.6110
score: 6.7
---

The younger of Redondela's two railway viaducts, inaugurated in 1884, carries a second rail line across the town at a slightly different angle to the first. Standing beneath the two structures simultaneously — which you can do from certain points in the old town — gives a vivid sense of how dramatically the 19th-century railway era transformed the landscape of this small Galician town.

The Pontevedra viaduct is slightly longer and slightly higher than its neighbour, though the two are so visually similar that visitors often mistake one for the other. Both are stone, both are listed, and both are still in daily active use — making Redondela one of the few places in Europe where two heritage viaducts run parallel through a town centre.
