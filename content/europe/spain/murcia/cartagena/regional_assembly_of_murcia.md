---
latitude: 37.6072
longitude: -0.9798
sources:
  - https://en.wikipedia.org/wiki/Regional_Assembly_of_Murcia
tags:
  - things_to_do
  - sight
  - architecture
title: Regional Assembly of Murcia
type: poi
---

The Regional Assembly of Murcia sits in Cartagena rather than the regional capital, a small political oddity that says something about the city's historic weight. The building is contemporary, but its waterfront position makes it part of the civic face Cartagena shows to the harbour.

You do not need a long visit here. Walk past on the way between the centre and the port, and note how the modern parliament joins naval facilities, museums, and older monuments along the same urban edge.
