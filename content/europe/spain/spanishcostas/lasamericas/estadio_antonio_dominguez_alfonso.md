---
latitude: 28.0597
longitude: -16.7261
sources:
  - https://en.wikipedia.org/wiki/Estadio_Antonio_Dom%C3%ADnguez_Alfonso
tags:
  - things_to_do
  - sport
title: Estadio Antonio Domínguez Alfonso
type: poi
---

The Estadio Antonio Domínguez Alfonso is the local sports ground in Playa de las Américas. It is not a conventional sightseeing stop, but it gives a glimpse of ordinary municipal life in a place better known for hotels and nightlife.

If there is a match or athletics event on, it can be an easy change of pace from the beach. Otherwise, pass it as part of a walk inland toward Central Park and the busier shopping streets.
