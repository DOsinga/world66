---
title: Saltimporten Canteen
type: poi
latitude: 55.6105
longitude: 12.9823
tags:
  - eating_out
  - restaurant
score: 6.6
---

One of the most talked-about lunch spots in Malmö: a canteen in the Western Harbour serving a single changing daily menu to a large open room of workers, architects, and students from the nearby university. The food is simple, well-sourced, and genuinely good — typically a salad, a main, and bread, often built around vegetables and grains in the New Nordic style without the expense.

The building is an old salt warehouse on the Dockan waterfront, with harbour views and the Turning Torso visible from outside. Lunch only, weekdays. Arrive before 12:30 or expect a queue. The canteen concept — one menu, one price, eat what you're given — works partly because the quality is consistently high and partly because the setting is so good.
