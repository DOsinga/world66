---
title: Engadine Museum
type: poi
tags:
  - things_to_do
  - museum
latitude: 46.4963
longitude: 9.8387
score: 7.2
---

The Engadine Museum (Museo Engiadinais) in St. Moritz Bad is housed in a reconstructed Engadine farmhouse and documents the distinctive culture and architecture of the valley: the sgraffito-decorated facades, the traditional dress, the farming implements, and the history of the area from prehistoric times through the development of tourism. The collection is modest but well-presented and contextualises the rather abstract luxury of modern St. Moritz.

The Romansh language and its dialects are given particular attention — maps of Romansh-speaking areas, examples of the written language, recordings of speakers. For most visitors this is the first encounter with a language that sounds simultaneously like Latin, Italian, and something entirely else. The museum is open year-round and is a good introduction to the valley's cultural depth.
