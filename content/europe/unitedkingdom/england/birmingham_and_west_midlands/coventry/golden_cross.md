---
latitude: 52.4078
longitude: -1.5086
snippet: One of Coventry's oldest pubs, in a 15th-century building near the guildhall
tags:
  - bars_and_cafes
  - bar
title: Golden Cross
type: poi
score: 6.9
---

The Golden Cross on Hay Lane is one of Coventry's oldest pubs, occupying a building that dates from the 15th century and survived the Blitz more or less intact. It sits just around the corner from St Mary's Guildhall and has the low ceilings and irregular floorplan you'd expect from a building this old. The pub has been serving drinks on this spot, in various forms, for more than five hundred years — which makes it a rarity in a city that lost so much of its medieval fabric.

The interior retains its period character, with original beams and a layout that hasn't changed significantly in decades. It serves real ales and straightforward pub food. Not a tourist trap — it functions as a proper local pub that happens to occupy a genuinely historic building. If you're walking between the guildhall and the cathedral, it makes an obvious stop.
