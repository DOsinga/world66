---
latitude: 51.2295
longitude: -3.8338
score: 9.1
sources:
- https://en.wikipedia.org/wiki/South_West_Coast_Path
tags:
- things_to_do
- sight
- cycling
- park
- wildlife
title: South West Coast Path
type: poi
---

The South West Coast Path passes through Lynmouth and runs in both directions along some of its most dramatic stretches. Heading west takes you along the cliff tops to the Valley of Rocks and beyond toward Combe Martin — a section with consistent sea views and a real sense of exposure when the weather comes in. Heading east from Lynmouth, the path climbs steeply before following the clifftop above the Foreland, the most northerly point on the Exmoor coast.

Both directions involve significant ascent from the valley floor. The path is well-maintained and waymarked. Even a short section — an hour out, an hour back — gives access to stretches of coast that are impossible to see any other way. This is one of the few places in England where the moor meets the sea directly, without any lowland buffer, and the walk makes that connection visceral.
