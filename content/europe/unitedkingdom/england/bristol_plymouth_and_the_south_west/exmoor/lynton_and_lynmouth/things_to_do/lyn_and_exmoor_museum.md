---
latitude: 51.2337
longitude: -3.8354
sources:
  - https://en.wikipedia.org/wiki/Lyn_and_Exmoor_Museum
tags:
  - things_to_do
  - museum
title: Lyn and Exmoor Museum
type: poi
score: 6.5
---

A small local museum in Lynton, housed in a cottage near the town hall, covering the history of the twin villages and the wider Exmoor area. The collection includes photographs and documents from before and after the 1952 flood, exhibits on the Victorian tourism that shaped the town, local geology, and the traditional industries of the moor — farming, charcoal burning, and fishing.

It won't take more than an hour, but it provides useful context for the landscape you're walking through. The staff are typically locals with real knowledge of the area. Open from Easter to October; check current hours before visiting, as it runs on volunteer help and occasionally keeps irregular times.
