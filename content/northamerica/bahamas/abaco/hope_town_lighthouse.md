---
title: "Hope Town Lighthouse"
type: poi
tags:
  - things_to_do
  - sight
  - hope_town
latitude: 26.5264
longitude: -76.9681
sources:
  - https://en.wikipedia.org/wiki/Elbow_Cay
story: >
  The Imperial Lighthouse Service completed Hope Town lighthouse in 1863 over the fierce
  objections of the local wreckers, who feared — correctly — that a proper light would
  end the lucrative business of salvaging ships that ran aground on Elbow Reef. According
  to local history, one wrecker bribed the construction crew to delay the project.
  The light went up anyway, and the wrecking trade died within a generation.
score: 8.6
---

The candy-striped lighthouse at Hope Town is the most photographed landmark in the Abacos and one of the most distinctive in the Caribbean. The red-and-white barber-pole tower rises 89 feet from the northern tip of Elbow Cay and has guided ships around the shallow reef since 1863. It is one of only a handful of manually-operated kerosene lighthouses still functioning anywhere in the world.

The climb is 101 steps up a spiral staircase. At the top, the mechanism — a French-made Fresnel lens mounted on a mercury float — still works by the same principle it always has. The keeper winds a weight, the weight descends, and the lens rotates. From the gallery you look out over the Sea of Abaco in one direction and the Atlantic in the other. The lighthouse is managed by the Bahamas Lighthouse Preservation Society and is open to visitors most mornings; the schedule varies, so check locally.
