---
title: "Palacio Legislativo"
type: poi
tags:
  - things_to_do
  - sight
  - architecture
latitude: 19.8441
longitude: -90.5312
score: 6.2
---

The Campeche state legislature sits in a building that looks as though it landed from a different century entirely. The Palacio Legislativo is a gleaming white modernist structure with a distinctive oval, disc-like form that locals call "el OVNI" — the UFO. Designed in the 1970s, it was inserted into the colonial grid of the historic centre with almost studied indifference to its surroundings.

The contrast is startling and, once you accept it, oddly charming. Campeche is a city that doesn't take itself entirely seriously, and the UFO building makes more sense in that context. It is visible from several streets of the historic centre and provides a useful orientation point. There is no public interior access, but the exterior is worth a brief detour.
