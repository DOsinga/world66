---
title: "Endless Wall Trail"
type: poi
snippet: The classic gorge-rim hike — 2.5 miles past climbers and overlooks.
tags:
  - things_to_do
  - sight
  - hiking
latitude: 38.0656
longitude: -81.0589
score: 9.1
---

The best short hike in the New River Gorge: 2.4 miles round trip along the top of the eponymous Endless Wall — a 250-foot Nuttall sandstone cliff that runs almost continuously for several miles along the north rim. The path winds through hemlock and rhododendron forest, with side trails out onto rocky points called "Diamond Point" and "Fern Buttress" that give clean drops straight into the gorge.

You'll likely see climbers on the wall below, sometimes within speaking distance. The trail is mostly easy, with one set of metal ladders descending a short cliff section near the start. Two trailheads (Fern Creek and Nuttall) allow a one-way shuttle hike.
