...
DIMENSIONS_FILE = DATA_DIR / "latent_label_scores.json"
...
def main():
    ...
    score_names = score_dimensions(dimensions)
    ...
    x = np.array(
        [
            [dimensions[path][dimension] for dimension in score_names] + hidden_by_path[path].tolist()
    ...
    args.model_out.write_text(
        json.dumps(
            {
                "alpha": args.alpha,
                "features": list(score_names) + [f"hidden_{i}" for i in range(hidden_data["hidden"].shape[1])],
