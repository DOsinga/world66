...
0. **Check loc_type and hierarchy placement** before anything else:
   ...
   - **If the parent country uses regions** (has a region level below it), every city and feature must live inside a region — not directly at the country level. **Exception**: globally recognised capitals and major cities (Paris, London, Berlin, Rome, Tokyo) may sit at the country level even when regions exist — do not move these. If a lesser city is at the country level incorrectly, note it for the country-specific cleanup PR rather than moving it here.
