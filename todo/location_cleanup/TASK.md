...
0. **Check loc_type and hierarchy placement** before anything else:
   ...
   - **If the parent country uses regions** (has a region level below it), every city and feature must live inside a region — not directly at the country level. **Exception**: globally recognised capitals and major cities (Paris, Berlin, Hamburg, Tokyo) may sit at the country level even when regions exist — do not move these. If a lesser city is at the country level incorrectly, note it for the country-specific cleanup PR rather than moving it here.
   - **UK exception**: The United Kingdom has four constituent countries (England, Scotland, Wales, Northern Ireland) that sit directly below `unitedkingdom/` and act as its region level. England is large enough to have its own sub-regions within it (`cumbria`, `eastern_england`, etc.), making it a legitimate two-level grouping below the country. This is a known structural exception — do not treat it as a hierarchy violation.
   - **Cities** are leaf-level settlements: they have POIs, sections, and neighbourhoods, but they do not contain other cities or features as child locations. **Features** are named areas that are destinations in their own right (national parks, gorges, coastlines, named tourist regions like Cinque Terre or Normandy) — they have their own POIs and nearby cities can tag into them via `tags: [feature_slug]`. **Regions** are the single grouping level directly below a country and contain cities and features.
   - If a location is tagged `loc_type: region` but sits *inside* a region rather than being the top-level grouping itself (Loire Valley, Côte d'Azur, Ardèche), change it to `loc_type: feature`. The one-level-below-country rule means these cannot be nested regions.
   - If a location is tagged `loc_type: region` and is the country's top-level grouping (California, Tuscany, North France), keep it as `region`.
