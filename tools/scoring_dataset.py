...
BATCH_DIR = SCORING_DIR / "batches"
AGENT_SCORE_DIR = SCORING_DIR / "agent_scores"
RESCORE_BATCH_DIR = SCORING_DIR / "rescore_batches"
RESCORE_DIR = SCORING_DIR / "rescores"
SCORED_FILE = SCORING_DIR / "scored.json"
ALL_LOCATIONS_FILE = SCORING_DIR / "all_locations.json"
...
def cmd_sample(args):
    ...
    print(f"Wrote {len(sample)} locations to {args.out.resolve().relative_to(PROJECT_DIR)}")
...
def cmd_expand_sample(args):
    ...
    if needed <= 0:
        print(f"{args.sample.resolve().relative_to(PROJECT_DIR)} already has {len(existing)} locations")
        return
    ...
    print(f"Expanded {args.sample.resolve().relative_to(PROJECT_DIR)} to {len(expanded)} locations")


def cmd_export_locations(args):
    locations = discover_locations()
    write_json(args.out, locations)
    print(f"Wrote {len(locations)} locations to {args.out.resolve().relative_to(PROJECT_DIR)}")
